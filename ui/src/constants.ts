export const API_URL = "http://localhost:5173/api";

export const TOKEN_KEY = "refine-auth";
export const IDENTITY_KEY = "refine-identity";

export const COMMON_DATA_PARENT_MENU = "Common";
export const MASTER_DATA_PARENT_MENU = "Master Data";
export const FINANCIALS_PARENT_MENU = "Financials";
export const SALES_PARENT_MENU = "Sales";
export const INVENTORY_HANDLING_PARENT_MENU = "Inventory Handling";
export const EMPLOYEE_MANAGEMENT_PARENT_MENU = "Employee Management";
export const RESTAURANT_MANAGEMENT_PARENT_MENU = "Restaurant Management";
export const USER_MANAGEMENT_PARENT_MENU = "User Management";
